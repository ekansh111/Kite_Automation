...
def save_sorted_to_csv(df, selected_date, output_directory, target_results_long_df, target_results_short_df):
    ...
    if not df_sorted.empty:
        ...
        cum_total_adj_stddev_pnl = average_stddev_adj__pnl_selected.sum() - FixedCommissionCost
    ...
    if not df_sorted_short.empty:
        ...
        #Adding the commision percent since in the shorting side the average return will be negative
...
# Example placeholders for your environment-specific variables
totalBatchSize = 500
...
def getSymbolsFromPostgresForDate(startDateStr):
    """
    Connects to Postgres, queries the 'nifty500_hist_const' table
    to find all tickers that were included in the Nifty 500 as of 'startDateStr',
    excluding any that had been removed before that date.
    
    Returns a list of ticker symbols (strings).
    """
    queryDate = datetime.strptime(startDateStr, "%Y-%m-%d")
    ...
    sqlQuery = f""" SELECT DISTINCT ticker_name_inclusion
                ...
                AND (
                    event_date_exclusion IS NULL
                    OR event_date_exclusion >= '{dateForSql}'
                    );
                """
    connection = None
    symbolsList = []
...
def main():
    ...
    # Decide how to get the symbol list
    if startDateObj < datetime(2021, 1, 1):
        print(f"Start date {startDateInput} is before 2021, fetching from Postgres table.")
        symbols = getSymbolsFromPostgresForDate(startDateInput)
        print(f"Fetched {len(symbols)} symbols from DB for date <= {startDateInput}.")
    else:
        print("Start date >= 2021, reading from CSV as before...")
        csvFilePath = Nifty500ConstituentList
        df = readCsvFile(csvFilePath, delimiter=',')
        if 'Symbol' in df.columns:
            symbols = df['Symbol'].dropna().unique().tolist()
            print(f"Total symbols found: {len(symbols)}")
        else:
            # Attempt fallback tab delimiter or reformat logic
            print("Attempting to read the CSV file with tab delimiter...")
            df = readCsvFile(csvFilePath, delimiter='\t')
            # ... etc. reformat as needed
            symbols = df['Symbol'].dropna().unique().tolist()
    ...
    # Validate the start and end dates
    try:
        start_date_obj = datetime.strptime(startDateInput, "%Y-%m-%d")
        end_date_obj = datetime.strptime(endDateInput, "%Y-%m-%d")
    ...
    target_results_long_file = os.path.join(output_directory, f'target_results_long_{startDateInput}_to_{endDateInput}.csv')
    target_results_short_file = os.path.join(output_directory, f'target_results_short_{startDateInput}_to_{endDateInput}.csv')
    
    target_results_long_df['pnl'] = (((target_results_long_df['returns'] - CommissionPercent) * CapitalRiskedPerLongTrade * NumberOfStocksToSelectLowestOpenPrice)/100) - FixedCommissionCost
    target_results_short_df['pnl'] = (((target_results_short_df['returns'] + CommissionPercent)* CapitalRiskedPerShortTrade * -1 *NumberOfStocksToSelectHighestOpenPrice)/100) - FixedCommissionCost
    try:
